...
<x-guest-layout>
    ...
    <form method="POST" action="{{ route('login') }}">
        ...
        <div>
            <x-input-label for="username" :value="__('Username')" />
            <x-text-input id="username" style="border-radius: 24px;" class="block mt-1 w-full sm:rounded-3xl" type="text" name="username"
        ...
        <div class="mt-4">
            <x-input-label for="password" :value="__('Password')" />
            <x-text-input id="password" style="border-radius: 24px;" class="block mt-1 w-full sm:rounded-3xl" type="password" name="password" required
                autocomplete="current-password" placeholder="Masukan Password..." />
            <x-input-error :messages="$errors->get('password')" class="mt-2" />
        </div>
